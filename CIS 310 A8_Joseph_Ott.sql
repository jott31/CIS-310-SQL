...
SELECT P.OFFICE_NUM, P.ADDRESS, P.MONTHLY_RENT, OW.OWNER_NUM, OW.FIRST_NAME, OW.LAST_NAME
	FROM OWNER OW INNER JOIN PROPERTY P ON P.OWNER_NUM = OW.OWNER_NUM;
...
SELECT P.PROPERTY_ID, P.OFFICE_NUM, P.ADDRESS, SR.EST_HOURS, SR.SPENT_HOURS, O.OWNER_NUM, O.LAST_NAME
	FROM SERVICE_CATEGORY SC INNER JOIN SERVICE_REQUEST SR ON SC.CATEGORY_NUM = SR.CATEGORY_NUMBER
							INNER JOIN PROPERTY P ON SR.PROPERTY_ID = P.PROPERTY_ID
							INNER JOIN OWNER O ON P.OWNER_NUM = O.OWNER_NUM
				WHERE SC.CATEGORY_DESCRIPTION = 'FURNITURE REPLACEMENT';
...
SELECT OWNER_NUM,FIRST_NAME, LAST_NAME
	FROM OWNER OW
		WHERE EXISTS (SELECT OWNER_NUM
								FROM PROPERTY P
									WHERE BDRMS = 2 AND OW.OWNER_NUM = P.OWNER_NUM);
...
SELECT P.ADDRESS, P.SQR_FT,P.OWNER_NUM,SR.SERVICE_ID,SR.EST_HOURS,SR.SPENT_HOURS, SR.CATEGORY_NUMBER
	FROM SERVICE_REQUEST SR INNER JOIN PROPERTY P ON SR.PROPERTY_ID = P.PROPERTY_ID
...
SELECT P.ADDRESS, P.SQR_FT,P.OWNER_NUM,SR.SERVICE_ID,SR.EST_HOURS,SR.SPENT_HOURS, SR.CATEGORY_NUMBER
	FROM SERVICE_REQUEST SR RIGHT JOIN PROPERTY P ON SR.PROPERTY_ID = P.PROPERTY_ID;
